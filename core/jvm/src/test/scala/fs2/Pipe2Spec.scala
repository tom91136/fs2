...
//      val interrupt = mkScheduler.flatMap { _.sleep[IO](20.millis) }.run.attempt
//      // tests that termination is successful once the scheduler signal interruption
...
//
//    "interrupt (3)" in  {
//      val s1 = Stream.constant(false)
//      val interrupt = mkScheduler.flatMap { _.sleep[IO](20.millis) }.run.attempt
//      // tests that interruption is successful even for infinite streams
...
//
//      //val s1 = Stream.range(0, 10) ++ Stream.range(20, 30)
//      println(s"XXX $s1")
