  ...
  def switchMap[F2[x] >: F[x], O2](f: O => Stream[F2, O2])(
    ...
    Stream.eval(Semaphore[F2](1)).flatMap { guard =>
      Stream.eval(Ref.of[F2, Option[Deferred[F2, Unit]]](None)).flatMap { haltRef =>
        this.evalMap { o =>
          Deferred[F2, Unit].flatMap(
            halt =>
              haltRef.modify {
                case None       => halt.some -> F2.unit
                case Some(last) => halt.some -> last.complete(())
              }.flatten *> F2.pure(
                Stream.eval(guard.acquire *> F2.delay(println(s"Do $o"))) >>
                  f(o)
                    .interruptWhen(halt.get.attempt)
                    .onFinalize(guard.release)))

        }.parJoinUnbounded.adaptError{case e :CompositeFailure => println(e.all.toList.mkString(","));e}
